...
class LatentPlanner:
    ...
    def planner_astar(self,start, goal):
        ...
        if self.cfg.ModelConfig.data_type == "visual":
            with torch.no_grad():
                z_start = self.model.encoder(start.unsqueeze(0).unsqueeze(0)).squeeze(0)
                z_goal = self.model.encoder(goal.unsqueeze(0).unsqueeze(0)).squeeze(0)
        else:
        ...
        print(f"\n[A* 诊断]: 起点 {np.round(start_tuple, 2)} -> 终点 {np.round(z_goal.numpy(), 2)}")
        print(f"初始距离: {h(z_start):.4f}")
